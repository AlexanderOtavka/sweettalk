...
test("lexFileWithLexers parses indents", t => {
  t.deepEqual(
    lexFileWithLexers("foo\n  if\n  foo", [fooLexer, ifLexer]),
    ok([
      { type: "foo" },
      { type: "indent", value: "  " },
      { type: "if" },
...
test("lexFileWithLexers handles windows line endings", t => {
  t.deepEqual(
    lexFileWithLexers("foo\r\n  if", [fooLexer, ifLexer]),
    ok([{ type: "foo" }, { type: "indent", value: "  " }, { type: "if" }]),
  )
})

test("lexFileWithLexers ignores indents of blank lines", t => {
  t.deepEqual(
    lexFileWithLexers("\n\n\n  foo\n  \n\n    if\n  \nfoo\n\n", [
      fooLexer,
      ifLexer,
